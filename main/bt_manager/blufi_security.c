...
static const char *TAG_BT_SEC = "mihome_esp32_bt_sec";
...
void blufi_dh_negotiate_data_handler(uint8_t *data, int len, uint8_t **output_data, int *output_len, bool *need_free)
{
    ...
    if (blufi_sec == NULL) {
        ESP_LOGI(TAG_BT_SEC, "BLUFI Security is not initialized");
    ...
    case SEC_TYPE_DH_PARAM_LEN:
        ...
        if (blufi_sec->dh_param == NULL) {
            ...
            ESP_LOGI(TAG_BT_SEC, "%s, malloc failed\n", __func__);
            return;
        }
        break;
    case SEC_TYPE_DH_PARAM_DATA:{
        if (blufi_sec->dh_param == NULL) {
            ESP_LOGI(TAG_BT_SEC, "%s, blufi_sec->dh_param == NULL\n", __func__);
        ...
        if (ret) {
            ESP_LOGI(TAG_BT_SEC, "%s read param failed %d\n", __func__, ret);
        ...
        if (ret) {
            ESP_LOGI(TAG_BT_SEC, "%s make public failed %d\n", __func__, ret);
            btc_blufi_report_error(ESP_BLUFI_MAKE_PUBLIC_ERROR);
            return;
        }

        mbedtls_dhm_calc_secret( &blufi_sec->dhm, blufi_sec->share_key, SHARE_KEY_BIT_LEN, &blufi_sec->share_len, NULL, NULL);
