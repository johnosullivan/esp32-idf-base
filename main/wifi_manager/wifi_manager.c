...
bool wifi_manager_fetch_wifi_sta_config(){
	...
	if(nvs_open(wifi_manager_nvs_namespace, NVS_READONLY, &handle) == ESP_OK){
		...
		ESP_LOGD(TAG, "wifi_manager_fetch_wifi_sta_config: ssid:%s password:%s",wifi_manager_config_sta->sta.ssid,wifi_manager_config_sta->sta.password);
...
void wifi_manager_connect_async(){
	...
	wifi_manager_send_message(WM_CONNECT_STA, (void*)CONNECTION_REQUEST_USER);
}
		...
		if( xStatus == pdPASS ){
			...
			case WM_LOAD_AND_RESTORE_STA:
				...
				if(wifi_manager_fetch_wifi_sta_config()){
					...
					wifi_manager_send_message(WM_START_AP, NULL);
				}
			...
			case EVENT_STA_DISCONNECTED:
				...
				if( uxBits & WIFI_MANAGER_REQUEST_STA_CONNECT_BIT ){
					...
					if(retries < WIFI_MANAGER_MAX_RETRY){
						retries++;
						wifi_manager_send_message(WM_CONNECT_STA, (void*)CONNECTION_REQUEST_AUTO_RECONNECT);
					}
					else{
